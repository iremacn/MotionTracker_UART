...
void setup() {
  Serial.begin(115200);
  STM32Serial.begin(115200, SERIAL_8N1, STM32_SERIAL_RX, STM32_SERIAL_TX);
  ...
  Serial.println("=== ESP32 DEBUG MODE ACTIVE ===");
  Serial.println("Serial Monitor Commands:");
  Serial.println("- Send JSON to test parsing");
  Serial.println("- Example: {\"device_id\":\"TEST\",\"motor_speed\":50,\"gyro_x\":1.2,\"gyro_y\":-0.8,\"gyro_z\":2.1,\"magnitude\":2.5,\"timestamp\":12345}");
}
...
void loop() {
  ...
  if (STM32Serial.available()) {
    ...
    if (stm32_data.length() > 0) {
      Serial.println("STM32 Data Received: " + stm32_data);
...
// STM32'den gelen verisini işle (SIMPLIFIED DEBUG VERSION)
void processSTM32Data(String data) {
  ...
  // HEX formatında veriyi göster (debug için)
  Serial.print("HEX data: ");
  for (int i = 0; i < data.length() && i < 50; i++) {  // İlk 50 karakteri göster
    Serial.print("0x");
  ...
  // ASCII karakterleri göster
  Serial.print("ASCII chars: ");
  ...
  // Basit format kontrolü: TEST:Motor=25,X=1.2
  ...
  if (!data.startsWith("{")) {
    Serial.println("ERROR: InvalidInput - Does not start with {");

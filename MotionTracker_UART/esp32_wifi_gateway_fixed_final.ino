...
void setup() {
  ...
  Serial.println("=== ESP32 UART FULL DEBUG MODE ===");
  ...
  Serial.println("UART FULL DEBUG MODE - Baud Rate: 9600");
  ...
  Serial.println("=== ESP32 UART FULL DEBUG MODE ACTIVE ===");
  ...
  Serial.println("Expecting A, B, C characters from STM32_Init()...");
  
  // İlk veri değerleri
  lastData.device_id = "STM32_WAITING";
...
void loop() {
  ...
  if (STM32Serial.available()) {
    ...
    Serial.println("=== RAW BYTES RECEIVED FROM STM32 ===");
  ...
  if (millis() - last_test_time > 15000) {  // Her 15 saniyede
    ...
    Serial.println("📤 Sending periodic test to STM32: " + test_msg);
...
// STM32'den gelen verisini işle (FULL DEBUG + DUAL FORMAT SUPPORT)
void processSTM32Data(String data) {
  Serial.println("=== PROCESSING STM32 DATA ===");
  ...
  // *** MOTOR FORMAT KONTROLÜ: MOTOR:25,X:1.2,Y:-0.8,Z:2.5 ***
  if (data.startsWith("MOTOR:")) {
    Serial.println("✅ MOTOR FORMAT DETECTED!");
    ...
    if (motorIndex > 0 && xIndex > motorIndex) {
      ...
      if (zIndex > yIndex) {
        ...
        Serial.println("📊 Gyro Y parsed: " + String(lastData.gyro_y));
      }
      ...
      if (zIndex > 0) {
        ...
        Serial.println("📊 Gyro Z parsed: " + String(lastData.gyro_z));
      }
      ...
      Serial.println("✅ MOTOR DATA PROCESSED SUCCESSFULLY!");
  ...
  if (data.startsWith("TEST:")) {
    ...
    lastData.device_id = "STM32_SIMPLE_TEST";
    lastData.gyro_y = -0.5;
    lastData.gyro_z = 2.1;
...
void handleRoot() {
  ...
  html += "<p>🔧 UART: 9600 baud (FULL DEBUG MODE) ✅</p>";
  ...
  html += "document.querySelector('.progress-text').innerText = speed + '%';";
  html += "document.querySelector('.progress-fill').style.width = speed + '%';";
  html += "fetch('/motor', {";
  html += "method: 'POST',";
  ...
  html += ".then(data => {";
  html += "console.log('Motor speed set to: ' + speed + '%');";
...
void handleData() {
  ...
  doc["uart_baud"] = 9600;
  doc["uart_status"] = "FULL_DEBUG_MODE_FIXED";

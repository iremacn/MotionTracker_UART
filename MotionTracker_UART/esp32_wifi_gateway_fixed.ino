...
const char* ssid = "WiFi_Aginizin_Adi";         // WiFi ağ adı
...
void setup() {
  ...
  Serial.println("✅ Route registered: GET /data");
  server.on("/motor", HTTP_POST, handleMotorControl);
  Serial.println("✅ Route registered: POST /motor");
  server.on("/style.css", handleCSS);
  Serial.println("✅ Route registered: GET /style.css");
  
  // 404 handler ekle
  server.onNotFound([]() {
    Serial.println("❌ 404 - Route not found: " + server.uri());
    String method = "UNKNOWN";
    if (server.method() == HTTP_GET) method = "GET";
    else if (server.method() == HTTP_POST) method = "POST";
    else if (server.method() == HTTP_PUT) method = "PUT";
    else if (server.method() == HTTP_DELETE) method = "DELETE";
  ...
  server.begin();
  Serial.println("🚀 Web Server Started with debug routes!");
  if (WiFi.status() == WL_CONNECTED) {
...
void loop() {
  ...
  // Web server debug - her request'i logla
  static unsigned long last_debug = 0;
  if (millis() - last_debug > 5000) {  // 5 saniyede bir
    last_debug = millis();
    Serial.println("🌐 Web Server Active - Waiting for requests...");
  }
...
void handleMotorControl() {
  ...
  Serial.println("Request method: " + method);
  Serial.println("Request URI: " + server.uri());
